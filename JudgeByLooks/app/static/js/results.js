...
async function init() {
  ...
  try {
    ...
    recommendations = normalizeScores(data.products || []);
  } catch (err) {
...
function normalizeScores(products) {
  ...
  const scores = products.map(p => p.similarity_score || 0);
  const min = Math.min(...scores);
  const max = Math.max(...scores);
  const range = max - min;
  return products.map(p => ({
    ...p,
    _display_score: range > 0.01
      ? 60 + Math.round(((p.similarity_score - min) / range) * 35)
...
function renderCards(products) {
  ...
  products.forEach(p => {
    const matchPct = p._display_score ?? Math.round((p.similarity_score || 0) * 100);
